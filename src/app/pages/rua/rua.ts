...
export class RuaComponent implements OnInit {
  ...
  carregarLista(): void {
    this.isLoading = true;
    this.apiService.getRuas().subscribe({
      next: (data) => {
        this.listaDeRuas = data;
        this.isLoading = false;
      },
      error: (err) => {
        console.error('Erro ao carregar lista de ruas:', err);
  ...
  salvar(): void {
    ...
    let observable;
    if (this.isEditMode) {
      observable = this.apiService.updateRua(this.formModel.id, this.formModel);
    } else {
      observable = this.apiService.salvarRua(this.formModel);
    }
  ...
  deletar(itemId: number): void {
    if (confirm('Tem certeza que deseja excluir este item?')) {
      this.apiService.deleteRua(itemId).subscribe({
        next: () => {
